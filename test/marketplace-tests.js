...
describe("Marketplace create/cancel/accept offer", function () {   
    ...
    it("Should create offer", async function () {
        const [owner, addr1, addr2, addr3, addr4] = await ethers.getSigners();
        const options = {value: ethers.utils.parseEther("1.0")}
        const listItemTx = await marketplace.connect(addr2).createOffer(1, nft.address, options); // CREATE OFFER
        expect(await nft.ownerOf(1)).to.equal(addr1.address);
    ...
    it("Should create offer 2", async function () {
        ...
        const listItemTx = await marketplace.connect(addr3).createOffer(1, nft.address, options); // CREATE OFFER
